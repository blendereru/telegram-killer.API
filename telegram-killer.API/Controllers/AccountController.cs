...
public class AccountController : ControllerBase
{
    ...
    [Authorize]
    [EndpointSummary(
        "The endpoint needed to log user out from the system. This requires both access token in the header and refresh token in the body.")]
    [EndpointDescription(
        "Logs out user from the system by deleting his refresh session from db. This requires access token in the Authorization header and refresh token in the body")]
    [ProducesResponseType<ValidationProblemDetails>(StatusCodes.Status400BadRequest, "application/problem+json")]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status401Unauthorized, "application/problem+json")]
    ...
    [Authorize]
    [EndpointSummary(
        "The endpoint needed to retrieve current user's information. Note: this requires user access token in the Authorization header")]
    [EndpointDescription(
        "Retrieves current user's information. This requires user access token in the Authorization header")]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status401Unauthorized, "application/problem+json")]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound, "application/problem+json")]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status500InternalServerError, "application/problem+json")]
    ...
    [Authorize]
    [EndpointSummary(
        "The endpoint needed to retrieve user information based on email. User requesting to retrieve information should be authorized")]
    [EndpointDescription(
        "The endpoint needed to retrieve user information based on email. User requesting to retrieve information should be authorized(e.g. Provide access token in Authorization header)")]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status401Unauthorized, "application/problem+json")]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound, "application/problem+json")]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status500InternalServerError, "application/problem+json")]
    [ProducesResponseType<GetUserInformationResponse>(StatusCodes.Status200OK, "application/json")]
